...
const EditPas = ({ open, onClose, onUpdated, platform }) => {
  const [formData, setFormData]           = useState({ pas_name: '', description: '', region: '', status: '', type: '', cpu: '', users: '', uptime: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || 'Failed to update platform. Please try again.';
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', px: 4, py: 4 }}>
          {showSuccess && (
            <Alert icon={<CheckCircleOutlineIcon />} severity="success" sx={{ mb: 3, borderRadius: '8px' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3.5 }}>
            ...
            <FormRow label="Platform Name" required>
              <TextField
                fullWidth name="pas_name" value={formData.pas_name} onChange={handleChange}
                placeholder="Enter Platform Name" error={!!errors.pas_name}
                helperText={errors.pas_name} size="small"
              />
            </FormRow>
            ...
            <FormRow label="Description" required alignTop>
              <TextField
                ...
                placeholder="Enter Description" multiline rows={3}
                error={!!errors.description} helperText={errors.description} size="small"
              />
            </FormRow>

            <FormRow label="Region" required>
              <FormControl fullWidth size="small" error={!!errors.region}>
                <Select name="region" value={formData.region} onChange={handleChange} displayEmpty
                  renderValue={(v) => v || <Box component="span" sx={{ color: '#9ca3af' }}>Select Region</Box>}>
            ...
            <FormRow label="Status" required>
              <FormControl fullWidth size="small" error={!!errors.status}>
                <Select name="status" value={formData.status} onChange={handleChange} displayEmpty
                  renderValue={(v) => v || <Box component="span" sx={{ color: '#9ca3af' }}>Select Status</Box>}>
            ...
            <FormRow label="Type" required>
              <FormControl fullWidth size="small" error={!!errors.type}>
                <Select name="type" value={formData.type} onChange={handleChange} displayEmpty
                  renderValue={(v) => v || <Box component="span" sx={{ color: '#9ca3af' }}>Select Type</Box>}>
            ...
            <FormRow label="CPU Cores" required>
              <TextField
                fullWidth name="cpu" type="number" value={formData.cpu} onChange={handleChange}
                placeholder="Enter CPU Cores" error={!!errors.cpu} helperText={errors.cpu} size="small"
                inputProps={{ min: 1, max: 128, step: 1 }}
                sx={{ '& input[type=number]::-webkit-inner-spin-button': { opacity: 1 }, '& input[type=number]::-webkit-outer-spin-button': { opacity: 1 } }}
              />
            </FormRow>

            <FormRow label="Active Users">
              <TextField
                fullWidth name="users" type="number" value={formData.users} onChange={handleChange}
                placeholder="Enter number of users" size="small"
            ...
            <FormRow label="Uptime">
              <TextField
                fullWidth name="uptime" value={formData.uptime} onChange={handleChange}
                placeholder="e.g. 99.9%" size="small"
              />
            </FormRow>
